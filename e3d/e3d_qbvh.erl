...
-export([ray/2, ray/4, ray_trace/2, intersect/2]).
...
-compile({inline, [shear_scale/3, bary_centric/6]}).
...
init(FaceData, Opts)  ->
    ...
    case proplists:get_value(no_binary, Opts, false) of
	false ->
	    {Tree, {_N, Tris}} = swap_prims(Nodes, PrimIndx, FaceData),
	    #{bb=>WorldBB, ns=>array:foldl(fun binary_node/3, <<>>, Tree), ts=>Tris, map=>[]};
	true ->
	    FaceMap = array:map(fun(_I, TF) -> locate_data(TF, FaceData) end, PrimIndx),
	    #{bb=>WorldBB, ns=>Nodes, ts=>FaceMap, map=>[]}
    end.
...
ray_trace(Ray = #ray{d=Dir}, #{ns:=Tree, ts:=Tris}) when is_binary(Tree) ->
    Swap = e3d_bv:inv_sign(Dir),
    bin_ray_trace(Ray, [0], #hit{}, Swap, Tree, Tris);
ray_trace(Ray = #ray{d=Dir}, #{ns:=Tree, ts:=Tris}) ->
    Swap  = e3d_bv:inv_sign(Dir),
    erl_ray_trace(Ray, [0], #hit{}, Swap, Tree, Tris).

%%--------------------------------------------------------------------
%% @doc Intersect two QBvh's
%% @end
%%--------------------------------------------------------------------

intersect(#{bb:=BB1,ns:=Ns1,ts:=Ts1}, #{bb:=BB2,ns:=Ns2,ts:=Ts2}) ->
    case e3d_bv:dist(BB1,BB2) of
	false -> false;
	_ -> intersect_1([{0,0}],Ns1,Ts1,Ns2,Ts2,[])
    end.
...
create_tmp_leaf(Parent, Child, Start, End, NodeBox, {Nodes, NNodes}) -> 
   #qnode{bboxes=BBs,children=Ch} = array:get(Parent, Nodes),
    ...
    Updated = #qnode{bboxes   = setelement(Child+1, BBs, NodeBox),
		     children = setelement(Child+1, Ch, {leaf, Quads, Start})},
...
swap_leaf({leaf, Quads, Start}, {NQuads, Prims0}, PrimIndx, Data) ->
...
erl_ray_trace(Ray, [{leaf, Quads, Offset}|Stack], Hit, Swap, Tree, Tris) ->
    {R,H} = qtri_intersect(Offset, Offset+Quads, Ray, Hit, Tris),
    erl_ray_trace(R,Stack,H,Swap,Tree,Tris);
erl_ray_trace(Ray, [NodeData|Stack], Hit, Swap, Tree, Tris) ->
    Node = array:get(NodeData, Tree),
    Visit = qbbox_ray_intersect(Ray, Node, Swap),
    S = add_stack(1, Visit, Node#qnode.children, Stack),
    erl_ray_trace(Ray, S, Hit, Swap, Tree, Tris);
erl_ray_trace(_, [], Hit, _, _, _) -> Hit.

qbbox_ray_intersect(Ray, #qnode{bboxes={B1,B2,B3,B4}}, InvSwap) ->
    {e3d_bv:hit(Ray,InvSwap,B1),e3d_bv:hit(Ray,InvSwap,B2),
     e3d_bv:hit(Ray,InvSwap,B3),e3d_bv:hit(Ray,InvSwap,B4)}.

qtri_intersect(Offset, Size, #ray{d=Dir}=Ray0, Hit0, Tris) when Offset < Size ->
    Order = order(Dir),
    Shear = shear(Dir,Order),
    {Index0, Get0} = array:get(Offset, Tris),
    {Ray1, Hit1} = hit_triangle(Ray0, Order, Shear, Get0(Index0), Hit0, Index0),
    {Index1, Get1} = array:get(Offset+1, Tris),
    {Ray2, Hit2} = hit_triangle(Ray1, Order, Shear, Get1(Index1), Hit1, Index1),
    {Index2, Get2} = array:get(Offset+2, Tris),
    {Ray3, Hit3} = hit_triangle(Ray2, Order, Shear, Get2(Index2), Hit2, Index2),
    {Index3, Get3} = array:get(Offset+3, Tris),
    {Ray, Hit} = hit_triangle(Ray3, Order, Shear, Get3(Index3), Hit3, Index3),
    qtri_intersect(Offset+1, Size, Ray, Hit, Tris);
qtri_intersect(_, _, Ray, Hit, _) ->
    {Ray,Hit}.

%% Woop JCGT 2(1)
%% http://jcgt.org/published/0002/01/05/paper.pdf
hit_triangle(#ray{o=Orig}=Ray, {_,_,Kz} = Order, {_,_,Sz} = Shear,
	     {TA,TB,TC}, Hit0, Face) ->
...
%%%%%%%%%%%%%%%%
% Intersect
intersect_1([{N1, N2}|Stack0], Ns1,Ts1,Ns2,Ts2, Acc) when is_integer(N1), is_integer(N2) ->
    Stack = qbbox_intersect(array:get(N1, Ns1),array:get(N2, Ns2),Stack0),
    intersect_1(Stack, Ns1,Ts1,Ns2,Ts2, Acc);
intersect_1([{{leaf, Qs1, Of1}, {leaf, Qs2, Of2}}|Stack], Ns1,Ts1,Ns2,Ts2, Acc0) ->
    Tris1 = collect_tris(Qs1, Of1, Ts1),
    Tris2 = collect_tris(Qs2, Of2, Ts2),
    Acc = intersect_tris(Tris1, Tris2, Acc0),
    intersect_1(Stack, Ns1,Ts1,Ns2,Ts2, Acc);
intersect_1([{{leaf, Qs1, Of1}, NodeIndex}|Stack], Ns1,Ts1,Ns2,Ts2, Acc) ->
    intersect_2([{collect_tris(Qs1, Of1, Ts1),NodeIndex}|Stack], Ns1,Ts1,Ns2,Ts2, Acc);
intersect_1([{NodeIndex, {leaf, Qs2, Of2}}|Stack], Ns1,Ts1,Ns2,Ts2, Acc) ->
    intersect_2([{collect_tris(Qs2, Of2, Ts2),NodeIndex}|Stack], Ns1,Ts1,Ns2,Ts2, Acc).

intersect_2(_Stack, _Ns1, _Ts1,_Ns2,_Ts2, _Acc) ->
    ok.

collect_tris(_,_,_) ->
    ok.

intersect_tris(_,_,_) ->
    ok.

qbbox_intersect(#qnode{bboxes={B1,B2,B3,B4}, children={C1,C2,C3,C4}},#qnode{bboxes=BBs},Stack0) ->
    Stack1 = qbbox_intersect_1(B1, BBs, C1, Stack0),
    Stack2 = qbbox_intersect_1(B2, BBs, C2, Stack1),
    Stack3 = qbbox_intersect_1(B3, BBs, C3, Stack2),
    qbbox_intersect_1(B4, BBs, C4, Stack3).

qbbox_intersect_1(BB, #qnode{bboxes={B1,B2,B3,B4}, children={C1,C2,C3,C4}}, C, Stack0) ->
    Stack1 = qbbox_intersect_2(e3d_bv:dist(BB,B1), C, C1, Stack0),
    Stack2 = qbbox_intersect_2(e3d_bv:dist(BB,B2), C, C2, Stack1),
    Stack3 = qbbox_intersect_2(e3d_bv:dist(BB,B3), C, C3, Stack2),
    qbbox_intersect_2(e3d_bv:dist(BB,B4), C, C4, Stack3).

qbbox_intersect_2(false, C1, C2, Stack) ->
    case is_empty(C1) orelse is_empty(C2) of
	true -> Stack;
	false -> [{C1,C2}|Stack]
    end;
qbbox_intersect_2(_, _, _, Stack) ->
    Stack.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


bin_ray_trace(Ray, [NodeData|Stack], Hit, Swap, Tree, Tris)
  when length(Stack) < 24 ->  %% Hardcoded limit in opencl code
    case is_leaf(NodeData) of
	false ->
	    Node = bin_get_node(NodeData, Tree),
	    Visit = bin_qbbox_intersect(Ray, Node, Swap),
	    S = add_stack(1, Visit, Node#qnode.children, Stack),
	    %% io:format("~p: ~p ~p~n",[?LINE, Node, Visit]),
	    bin_ray_trace(Ray, S, Hit, Swap, Tree, Tris);
	true ->
	    QuadPrims = no_quads(NodeData),
	    Offset    = first_quad(NodeData),
	    {R,H} = bin_qtri_intersect(Offset, Offset+QuadPrims, Ray,Hit, Tris),
	    bin_ray_trace(R,Stack,H,Swap,Tree,Tris)
    end;
bin_ray_trace(_, [], Hit, _, _, _) -> Hit.

bin_qbbox_intersect(#ray{o={X,Y,Z}, n=N,f=F},
...
add_stack(N, Visit, Chs, Stack) when N < 5 ->
    case element(N, Visit) of
	true ->
	    case is_empty(Ch = element(N, Chs)) of
		true  -> add_stack(N+1, Visit, Chs, Stack);
		false -> add_stack(N+1, Visit, Chs, [Ch|Stack])
	    end;
	false ->
	    add_stack(N+1, Visit, Chs, Stack)
    end;
add_stack(_, _, _, Stack) -> Stack.

bin_qtri_intersect(Offset, Size, Ray0, Hit0, Tris)
  when Offset < Size ->
    Tri = get_qtriangles(Offset, Tris),
    {Ray,Hit} = bin_qtri_intersect(Ray0, Tri, Hit0),
    bin_qtri_intersect(Offset+1, Size, Ray, Hit, Tris);
bin_qtri_intersect(_, _, Ray, Hit, _) -> {Ray, Hit}.

bin_qtri_intersect(#ray{d={DirX,DirY,DirZ},
			o={Ox,Oy,Oz},
			n=Near,f=Far} = Ray0,
		   #qtri{v  = {{Tx1,Tx2,Tx3,Tx4},
			       {Ty1,Ty2,Ty3,Ty4},
			       {Tz1,Tz2,Tz3,Tz4}},
			 e1 = {E1X,E1Y,E1Z},
			 e2 = {E2X,E2Y,E2Z},
    ...
    Dx = {Ox-Tx1, Ox-Tx2, Ox-Tx3,Ox-Tx4},
    Dy = {Oy-Ty1, Oy-Ty2, Oy-Ty3,Oy-Ty4},
    Dz = {Oz-Tz1, Oz-Tz2, Oz-Tz3,Oz-Tz4},
    ...
    case check_qtris(1,Div,B0,B1,B2,T,Fs,Near,Far,undefined) of
	undefined -> {Ray0,Hit0};
	Hit -> {Ray0#ray{f=Hit#hit.t},Hit}
    end.

check_qtris(I, Ds,B0s,B1s,B2s,Ts,Ids,N,F,Prev) when I < 5 ->
    D  = element(I, Ds),
    B0 = element(I, B0s),
    B1 = element(I, B1s),
    B2 = element(I, B2s),
    T  = element(I, Ts),
    if D =/= 0.0, B0 >= 0.0, B1 >= 0.0, B2 >= 0.0, T > N, T < F ->
	    Id = element(I, Ids),
	    Hit = #hit{t=T,b1=B1,b2=B2,f=Id},
	    check_qtris(I+1,Ds,B0s,B1s,B2s,Ts,Ids,N,T,Hit);
       true ->
	    check_qtris(I+1,Ds,B0s,B1s,B2s,Ts,Ids,N,F,Prev)
    end;
check_qtris(_,_,_,_,_,_,_,_,_,Hit) -> Hit.
...
print_child({{leaf, _Quads, Start},BB}, _, PrimIndx, PBB, Level) ->
    check_bb(PBB,BB),
